...
session_start();
if (isset($_SESSION['id_usuario']) && $_SESSION['nivel'] == 1) {
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = mysqli_query($conexao, "SELECT * FROM produto WHERE id_produto = '$id'");
...
<body class="container mt-5">
    ...
    <form action="processa_produtos.php" method="post" enctype="multipart/form-data">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" value="<?php echo $dados['nome_produto']; ?>" id="nome" name="nome"
                required>
        </div>
        <div class="mb-3">
            <label for="descricao" class="form-label">Descrição do Produto</label>
            <textarea class="form-control" value="<?php echo $dados['descricao'] ?>" id="descricao" name="descricao"
                rows="3" required><?php echo $dados['descricao']; ?></textarea>
        </div>
        <div class="mb-3">
            <label for="preco" class="form-label">Preço do Produto</label>
            <input type="number" class="form-control" value="<?php echo $dados['preco']; ?>" id="preco" name="preco"
                step="0.01" required>
        </div>
        <div class="mb-3">
